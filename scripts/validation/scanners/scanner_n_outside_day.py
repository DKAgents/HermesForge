#!/usr/bin/env python3
"""
scanner_n_outside_day.py — STR-N: Outside Day Key Reversal

Standalone scanner for Phase 1A validation. NOT added to live registry.

Entry: ATR/Close > 1.5x its 50-day avg (elevated vol) + 2+ consecutive
      down days + outside day (engulfs prior range) + key reversal close
      (close above prior close) + volume >= 1.5x average + price above SMA200
Exit: Stop at outside day low, 3:1 R:R target (50% at 2:1), time stop 12 bars
Direction: Long-only
Regime: High-volatility (outside day reversal after decline)

Usage: python3 scanner_n_outside_day.py [--json]
"""
...
STRATEGY_ID = "STR-N-outside-day-key-reversal"
ATR_PERIOD = 14
VOL_LOOKBACK = 50
VOL_THRESHOLD = 1.5      # ATR/Close > 1.5x its 50-day average
DECLINE_DAYS = 2          # 2+ consecutive down days
VOLUME_MULT = 1.5         # volume >= 1.5x average
SMA_PERIOD = 200
...
def scan_ticker(df: pd.DataFrame, ticker: str) -> list:
    """Scan a single ticker for outside day key reversal signals."""
    signals = []
    ...
    # Normalized ATR for volatility regime detection
    ...
    for i in range(max(SMA_PERIOD, VOL_LOOKBACK) + DECLINE_DAYS + 2, len(df) - TIME_STOP_BARS):
        # 1. Elevated volatility regime filter
        ...
        # 2. Short-term decline (2+ consecutive down days ending at i-1)
        decline = True
        ...
        # 3. Outside day: today's high > prior high AND today's low < prior low
        if df['high'].iloc[i] <= df['high'].iloc[i - 1]:
            continue
        if df['low'].iloc[i] >= df['low'].iloc[i - 1]:
            continue
        
        # 4. Key reversal: close above prior close (after making a new low for decline)
        ...
        # Simulate with partial exit at 2:1, remainder at 3:1
        exit_price = None
        exit_reason = None
        exit_date = None
        ...
        signals.append({
            ...
            'outside_range': round(df['high'].iloc[i] - df['low'].iloc[i], 2),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="STR-N Outside Day Key Reversal Scanner")
    ...
    if signals:
        ...
        print(f"\n{'='*60}")
        print(f"STR-N Outside Day Key Reversal — Phase 1A Results")
        print(f"{'='*60}")
