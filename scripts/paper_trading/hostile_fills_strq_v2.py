#!/usr/bin/env python3
"""
hostile_fills_strq_v2.py — V2: True Limit Entry (fill at signal price, not bar open).
...
Rules (V2 — True Limit Entry):
  1. Signal on bar t → fill at signal entry price (not bar open), only if bar
     trades THROUGH entry price (true limit order — exact price or no fill).
  ...
  6. Output to hostile_fill_report_strq_v2.jsonl — never touches trades.csv.

Usage:
    python3 hostile_fills_strq_v2.py                    # run full comparison
    python3 hostile_fills_strq_v2.py --dry-run           # show what would be written
    python3 hostile_fills_strq_v2.py --stocks-only
    python3 hostile_fills_strq_v2.py --crypto-only
    python3 hostile_fills_strq_v2.py --force             # clear report and rescore
"""
...
ENTRY_MODE = "limit"         # V2: fill at signal price, not bar open (baseline: "market")
REPORT_PATH = pathlib.Path(__file__).parent / "hostile_fill_report_strq_v2.jsonl"
...
def check_entry_hostile(trade: dict, bars: pd.DataFrame) -> dict:
    ...
    for _, row in bars.iterrows():
        ...
        # V2: True limit order — fill at signal price, not bar open
        if _trades_through(row, entry_price, side):
            fill_price = _apply_taker_fee(entry_price, is_entry=True)
            return {
                "filled": True,
                "fill_price": fill_price,
                "fill_bar_idx": int(row["bar_idx"]),
                "fill_bar_date": str(row["date"])[:16],
                "fill_reason": f"limit fill at signal price, bar {row['bar_idx']}",
            }
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"STR-Q Hostile Fill Model V2 (Limit Entry) — {datetime.datetime.now(datetime.timezone.utc).isoformat()}")
