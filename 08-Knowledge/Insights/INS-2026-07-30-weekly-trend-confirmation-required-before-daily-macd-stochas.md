---
type: insight
date: 2026-07-30
actionability: 4
connection_type: creates_filter
domains: [concepts, indicators, rules]
sources: ["R142-weekly-signals-as-trend-filters-for-macd-and-stochastics", "C130-the-weekly-rule-price-channel-as-trend-following-alternative", "N044-long-term-moving-averages-on-weekly-charts"]
seed_id: trend_filter_entry
tags: [insight, discovery, knowledge-evolution]
topic: knowledge
confidence: high
has_quotes: false
source: unknown
---
# Weekly Trend Confirmation Required Before Daily MACD/Stochastics Entry

## Discovery Summary

R142 establishes that MACD and Stochastics daily crossovers must be filtered by weekly trend direction, but does not specify how to define that weekly trend. N044 provides concrete weekly trend definition tools — the 10/40-week MA combination — which can operationalize the filter R142 requires. C130 adds an alternative weekly trend definition via price channel breakouts, giving traders two distinct methods to establish the weekly trend that R142 mandates as a prerequisite.

## Trading Implication

Before acting on a daily MACD or Stochastics crossover signal, a trader should confirm that the weekly trend is aligned using either the 10/40-week moving average relationship (N044) or a weekly price channel breakout (C130) — only take daily crossover signals in the direction of the confirmed weekly trend.

## Supporting Notes

- [[R142-weekly-signals-as-trend-filters-for-macd-and-stochastics]]
- [[C130-the-weekly-rule-price-channel-as-trend-following-alternative]]
- [[N044-long-term-moving-averages-on-weekly-charts]]

## Connection Type

**creates_filter** — Actionability score: 4/5

## Related
- [[R268-technical-analysis-checklist-market-analysis-phase]] — Use the Market Analysis Phase checklist to define weekly trend direction.

- [[N065-macd-indicator-construction]] — See N065-macd-indicator-construction for the daily MACD structure that requires weekly trend filtering

- [[EN023-trendline-break-confirmation-of-major-trend-change]] — See EN023-trendline-break-confirmation-of-major-trend-change for trendline break method of weekly trend definition

- [[N067-macd-histogram-as-early-warning-signal]] — Weekly MACD histogram as trend filter
